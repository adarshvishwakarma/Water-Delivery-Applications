...
public class CurrentOrderActivity extends AppCompatActivity{
    ...
    private String shopUid,uid,shopPhoneNum,shopName;
    private double mUserLatitude,mUserLongitude,mShopLatitude,mShopLongitude;
    ...
    private void fetchInfo() {
        ...
        db.collection(SHOP_LIST).document(shopUid).get().addOnCompleteListener(task -> {

            if (task.isSuccessful()){
                DocumentSnapshot documentSnapshot = task.getResult();
                assert documentSnapshot != null;
                mShopLatitude = (double) documentSnapshot.get("latitude");
                mShopLongitude = (double) documentSnapshot.get("longitude");
                shopPhoneNum = (String) documentSnapshot.get("shop_phonenumber");
                shopName = (String) documentSnapshot.get("shop_name");
                db.collection(USER_LIST).document(uid).get().addOnCompleteListener(task1 -> {
                    if (task1.isSuccessful()){
                        ...
                        String time = String.valueOf(documentSnapshot1.get("shop_delivery_time"));
                        int shopDeliveryTime = Integer.parseInt(time);
                        mCountDownTimer = findViewById(R.id.easyCountDownTextview);
                        mTimeFormatText = findViewById(R.id.timeLeftText);
                        mCountDownTimer.setTime(0,0, shopDeliveryTime,0);
                        ...
                        init(mShopLongitude,mShopLatitude,mUserLongitude,mUserLatitude,shopName,shopPhoneNum);
                    }
    ...
    private void init(double mShopLongitude, double mShopLatitude,double mUserLongitude, double mUserLatitude, String shopName, String shopPhoneNum) {
        TextView OrderedShopName = findViewById(R.id.orderedShopName);
        TextView mCurrentOrderText = findViewById(R.id.currentOrderShopName);
        mCurrentOrderText.setText(shopName);
        OrderedShopName.setText(shopName);
        Button mCallShopBtn = findViewById(R.id.callShopBtn);
        mCallShopBtn.setOnClickListener(view -> {
            Intent intent = new Intent(Intent.ACTION_DIAL);
            intent.setData(Uri.parse("tel:" + shopPhoneNum));
            startActivity(intent);
        });

        Point origin = Point.fromLngLat(mShopLongitude, mShopLatitude);
        Point destination = Point.fromLngLat(mUserLongitude,mUserLatitude);
        LatLng loc1 = new LatLng(mUserLatitude, mUserLongitude);
        LatLng loc2 = new LatLng(mShopLatitude, mShopLongitude);
